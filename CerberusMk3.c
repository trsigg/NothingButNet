...
#pragma config(Motor,  port7,           BLeft,         tmotorVex393_MC29, openLoop)//6 motor drive
...
int DeltaE = 0; //How much less wrong I am
int power = 0;
float Flyspeed = 0;
int TargetSpeed;
int PrevError;
...
int Flypower = 0;
int powerlevel;
float Kps[5] = {0, 2.4, 2.4, 2.4, 2.4};
float Kis[5] = {0, 0.001, 0.001, 0.001, 0.001};
float Kds[5] = {0, 2.5, 2.5, 2.5, 2.5};
int TargetSpeeds[5] = {0, 205, 245, 183, 191};//Off, Skillz, Long, 1st, 2nd
int setpowers[5] = {0, 70, 100, 65, 68};
...
task motorcontrol()
{
	while(1)
	{
		...
		power = abs(setpower + Kp*Error + Ki*Integral + Kd*DeltaE);
		SensorValue[Flywheel] = 0;
		PrevError = Error;
		wait1Msec(25);
	}
